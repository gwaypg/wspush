...
var (
	...
	// 日志配置信息
	// 日志输出的级别
	level = proto.Level(etc.Etc.Int64("module/log", "level"))

	// 产生日志地方的客户端信息，用于日志服务器识别来源
	ctx = &proto.Context{
		Platform: etc.Etc.String("module/log", "platform"),
		Version:  etc.Etc.String("module/log", "version"),
		Ip:       logger.HostName,
	}

	// 日志输出适配器，用于输出日志
	adapter = []proto.Adapter{
		stdio.New(os.Stdout, os.Stderr),
	}

	// 系统默认输出前缀
	lg = logger.New(ctx, "", 4, level, adapter...)
)

// 快速构建一个带前缀的日志
func New(prefix string) proto.Logger {
	return logger.New(ctx, prefix, 3, level, adapter...)
}

// 发送行为行为日志信息, 一般不在控制台输出, 只记录在相关文件中
